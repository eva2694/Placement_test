from azure.storage.blob import BlobServiceClient, BlobClient, ResourceNotFoundError
import pandas as pd
...
def load_csv_from_blob(storage_account_name, storage_account_key, container_name, blob_name, encoding='utf-8'):
    ...
    try:
        ...
        for blob in paginator:
            ...
            with blob_client.open_read() as blob_data:
                # Load CSV data, handling large files in chunks.
                if blob_data.properties.size > 100 * 1024 * 1024:
                    for df_chunk in pd.read_csv(StringIO(blob_data.readall().decode(encoding)), chunksize=10**6):
                        dataframe_chunks.append(df_chunk)
                else:
                    dataframe_chunks.append(pd.read_csv(StringIO(blob_data.readall().decode(encoding))))
